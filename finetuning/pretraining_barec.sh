...
# Settings
export DATA_DIR="CAMeL-Lab/BAREC-Shared-Task-2025-sent"
export FALLBACK_FONTS_DIR="data/fallback_fonts"  # let's say this is where we downloaded the fonts to
export MODEL="bensapir/pixel-barec" # also works with "bert-base-cased", "roberta-base", etc.
export BSZ=64
export GRAD_ACCUM=1
export LR=1.5e-4
export SEED=42
...
python scripts/training/run_pretraining.py \
  --train_dataset_names="CAMeL-Lab/BAREC-Shared-Task-2025-sent" \
  --validation_dataset_name="CAMeL-Lab/BAREC-Shared-Task-2025-sent" \
  --dataset_cache="data/BAREC-Shared-Task-2025-sent" \
  --train_splits="train" \
  ...
  --report_to=wandb \
  --push_to_hub \
  --hub_model_id="bensapir/pixel-barec" \
  --hub_strategy="checkpoint" \
  --config_name="bensapir/pixel-barec" \
